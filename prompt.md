在已有测试代码基础上，对上线的系统进行全面的测试，修改测试中的问题，直到测试全部通过，循环执行本测试修改指令10次，直到所有测试通过；在现有平台中加入上传角色模板等数字资产的功能，即AI短剧工具里加入上传角色模板的功能；、新生成首帧图，以荀彧劝说曹操迎献帝的故事生成一个15秒的720p测试视频，新的测试的视频，成功的都放在示例视频里；真实画风；
...
采用全国产化方案，修改整个技术实现，采用无服务器架构，上线部署本系统，并测试所有功能通过；提交到github；
各种已经经过验证测试通过的的api key，帮我写进window的全局存储中，以便我在不同的项目中使用这些api key（我有几十个项目想需要使用这些keys）；
我有使用火山引擎最新的漫剧的api进行全面的技术实现的改造，升级本系统，全用火山引擎最新的漫剧专用的api实现，尽量用集成度最高的api，用漫剧专用接口，以提高质量和方便性；
全程指导我上线，让我越简单越好，你能帮我操作的全部帮我操作，然后实在不能的想办法全局安装工具帮我操作，没有mcp或者其他工具可实现的，告诉我详细步骤和链接；
